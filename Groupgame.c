		...
		for (int j = 0; j < 4; j++) {
			for (int k = 0; k < 4; k++) {
				if (g[i].points[j] > g[i].points[k]) {
					strcpy(temp_1, g[i].teamName[j]);
					strcpy(g[i].teamName[j], g[i].teamName[k]);	
					strcpy(g[i].teamName[k], temp_1);
					temp_2 = g[i].points[j];
					g[i].points[j] = g[i].points[k];
					g[i].points[k] = temp_2;
					temp_3 = g[i].goals[j];
					g[i].goals[j] = g[i].goals[k];
					g[i].goals[k] = temp_3;
				} else if (g[i].goals[j] > g[i].goals[k]) {
					strcpy(temp_1, g[i].teamName[j]);
					strcpy(g[i].teamName[j], g[i].teamName[k]);	
					strcpy(g[i].teamName[k], temp_1);
					temp_2 = g[i].points[j];
...
int main() {
    ...
    do {
        printf("MENU\n");
        printf("1. Read input\n");
        printf("2. Print the league table\n");
